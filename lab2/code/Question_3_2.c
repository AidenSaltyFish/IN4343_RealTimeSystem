...
// sudo sysctl kernel.sched_rr_timeslice_ms=xx
...
static void* Thread(void *inArgs)
{
	...
	struct timespec interval, next, rem, now;
    interval.tv_sec = 0;
    interval.tv_nsec = results[tid].thread_period * 1000;
	...
	while (a++ < 6) {
		...
		trace_write("RTS_Thread_%d Job %d arrived at Time: %lld", args->thread_number, a, getTimeStampMicroSeconds());
		...
		*/
		...
		//timespec_add_us(&(args->next), args->thread_period);

		results[tid].thread_deadline 		+= results[tid].thread_period;
        ...
        rem = timespec_sub(now, next);
        
        nanosleep(&interval,0);
		//clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &(args->next), NULL);
		//trace_write("RTS_Thread_%d Job %d sleep finish: %lld", args->thread_number, a, getTimeStampMicroSeconds());
	}

	// /* Following sequence of commented instructions should be filled at the end of each periodic iteration*/
	// results[tid].thread_deadline 		+= results[tid].thread_period;
	// results[tid].thread_response_time 	= (results[tid].thread_end_time.tv_sec 
	// 											- results[tid].thread_start_time.tv_sec) * 1000;
...
int main(int argc, char **argv)
{
	...
	int priorities[NUM_THREADS] = { 25, 50, 50, 50, 99}; //25, 50, 50, 50, 99
	/* Priorities are in the range from 1 to 99 when it comes to SCHED_FIFO and SCHED_RR, and for  
	SCHED_OTHER, the priority is always 0 */

	int periods[NUM_THREADS] = { 32000, 32000, 32000, 32000, 64000 };	// Used in calculation of next period value in a periodic task / thread.
	// in microsecond
	...
	for(int i = 0; i < NUM_THREADS; i++)
	{
		...
		clock_gettime(CLOCK_REALTIME, &(lvargs[i].next));
